...
For example,  the Chandlee & Jardine paper has the output function: <img src="https://render.githubusercontent.com/render/math?math=H_o(x) = \text{if}~ \square'(x) ~ \text{then} ~ \top ~ \text{else} ~ H(x)"> 
