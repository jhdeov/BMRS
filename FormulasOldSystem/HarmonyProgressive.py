...
Progressive harmony: the first vowel's feature spreads
...
def personal_predicate_setup(self):
    ...
    self.predicates_list =  ['initial', 'initial C span', 'first +', 'first -', 'after first +', 'after first -','after opaque B']
    ...
    if copy is 1 and label == '-':
        if self.get_value('input', 'predicate', 'first -', domain_element): return True
            ...
            for nonopaque in ['+','-']:
                if self.get_value('input','label',nonopaque,domain_element):
                    if self.get_value('input', 'predicate', 'after first -', domain_element): return True
                    if self.get_value('input','predicate','after opaque B',domain_element): return True
            else: return False
