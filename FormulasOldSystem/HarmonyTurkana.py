...
There's an opaque - suffix that I mark as B
B blocks all + spread and makes everything surface as -
/a/ surfaces as [O] if have + before it and B after it
Underlying value:
...
def personal_setup(self):
    ...
    self.input_symbols = ['+','-','T','a','o','O','B', 'C']
...
def personal_predicate_setup(self):
    ...
    self.predicates_list =  ['before +','after +','before opaque a','after opaque a','before opaque B']
...
def personal_Output_Formula(self, copy, label, domain_element):
    ...
    if copy is 1 and label == '+':
        if self.get_value('input','label','+',domain_element):
            ...
            if self.get_value('input', 'predicate', 'before opaque B', domain_element): return False
            elif self.get_value('input','predicate','before +',domain_element): return True
    ...
    if copy is 1 and label == 'O':
        if self.get_value('input','label','a',domain_element):
            if self.get_value('input','predicate','after +',domain_element):
    ...
    if copy is 1 and label == 'a':
        if self.get_value('input','label','a',domain_element):
            ...
            elif self.get_value(1,'label','O',domain_element): return False
            else: return True
