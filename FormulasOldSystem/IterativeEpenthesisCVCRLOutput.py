...
def personal_Output_Formula(self, copy, label, domain_element):
    ...
    if copy is 1 and label == "onset":
        if self.get_value('input', 'predicate', "C", domain_element):
            ...
            elif self.get_value('input', 'predicate', "C", succ):
                if self.get_value(1, 'label', "onset", succ_of_succ): return True
        else: return False
    #Generate codas based on onsets
    if copy is 1 and label == "coda":
        if self.get_value('input', 'predicate', "C", domain_element):
            if not self.get_value(1, 'label', "onset", domain_element):
                if self.get_value('input', 'predicate', "final", domain_element): return True
                elif self.get_value(1, 'label', "onset", succ): return True
        else: return False
